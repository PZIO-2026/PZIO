...
def create_work_item(db: Session, project_id: int, task: schemas.WorkItemCreate):
...
def get_work_items(db: Session, project_id: int, status: str = None, assignee_id: int = None, sprint_id: int = None, task_type: str = None):
...
def get_work_item(db: Session, task_id: int):
    return db.query(models.WorkItem).filter(models.WorkItem.id == task_id).first()

def update_work_item(db: Session, task_id: int, update_data: schemas.WorkItemUpdate):
...
def delete_work_item(db: Session, task_id: int):
    db_item = get_work_item(db, task_id)
    if not db_item:
        return False
    db.delete(db_item)
...
def update_work_item_status(db: Session, task_id: int, new_status: str, user_id: int):
    db_item = get_work_item(db, task_id)
    if not db_item:
        return None
...
def create_time_log(db: Session, task_id: int, log_data: schemas.TimeLogCreate, user_id: int):
...
def get_time_logs(db: Session, task_id: int):
